...
const EINKAUFSLISTE_FIELDS = [
  { key: 'listenname', label: 'Name der Einkaufsliste', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
];
const EINKAUFSEINTRAG_FIELDS = [
  { key: 'einkaufsliste_ref', label: 'Einkaufsliste', type: 'applookup/select', targetEntity: 'einkaufsliste', targetAppId: 'EINKAUFSLISTE', displayField: 'listenname' },
  { key: 'artikelname', label: 'Artikelname', type: 'string/text' },
  { key: 'zugeordneter_benutzer', label: 'Zugeordneter Benutzer', type: 'lookup/select', options: [{ key: 'akm', label: 'AKM' }, { key: 'benutzer_2', label: 'Benutzer 2' }, { key: 'benutzer_3', label: 'Benutzer 3' }] },
  { key: 'erledigt', label: 'Erledigt', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'einkaufsliste', label: 'Einkaufsliste', pascal: 'Einkaufsliste' },
  { key: 'einkaufseintrag', label: 'Einkaufseintrag', pascal: 'Einkaufseintrag' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('einkaufsliste');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'einkaufsliste': new Set(),
    'einkaufseintrag': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'einkaufseintrag': return (data as any).einkaufseintrag as Einkaufseintrag[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'einkaufseintrag' && fieldKey === 'einkaufsliste_ref') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'einkaufseintrag': return EINKAUFSEINTRAG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'einkaufseintrag': return {
        create: (fields: any) => LivingAppsService.createEinkaufseintragEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateEinkaufseintragEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteEinkaufseintragEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'einkaufseintrag' || dialogState?.entity === 'einkaufseintrag') && (
        <EinkaufseintragDialog
          open={createEntity === 'einkaufseintrag' || dialogState?.entity === 'einkaufseintrag'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'einkaufseintrag' ? handleUpdate : (fields: any) => handleCreate('einkaufseintrag', fields)}
          defaultValues={dialogState?.entity === 'einkaufseintrag' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'einkaufseintrag' && (
        <EinkaufseintragViewDialog
          open={viewState?.entity === 'einkaufseintrag'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'einkaufseintrag', record: r }); }}
          einkaufslisteList={(data as any).einkaufsliste ?? []}
        />
      )}
